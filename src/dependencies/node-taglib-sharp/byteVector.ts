...
export class ByteVector {
    ...
    public static concatenate(...vectors: Array<Uint8Array | ByteVector | number | undefined>): ByteVector {
        ...
        for (const v of vectors) {
            ...
            if (typeof v === "number") {
                ...
                const getter = v instanceof ByteVector ? (i: number) => v.get(i) : (i: number) => v[i];
     ...
     */
    public static fromByteArray(bytes: Uint8Array | Buffer | number[], length: number = bytes.length): ByteVector {
     ...
     */
    public static fromString(text: string, type: StringType, length: number = Number.MAX_SAFE_INTEGER): ByteVector {
        ...
        const vector = type === StringType.UTF16 ? new ByteVector(new Uint8Array([0xff, 0xfe])) : ByteVector.empty();
    ...
    public static getTextDelimiter(type: StringType): ByteVector {
        return type === StringType.UTF16 || type === StringType.UTF16BE || type === StringType.UTF16LE ? ByteVector.TD2 : ByteVector.TD1;
    }
    ...
    public toBase64String(): string {
        return Buffer.from(this._bytes.buffer, this._bytes.byteOffset, this._bytes.byteLength).toString("base64");
    }
    ...
    public toString(type: StringType): string {
        const bom = type === StringType.UTF16 && this.length > 1 ? this.subarray(0, 2) : undefined;
